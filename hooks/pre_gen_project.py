...
TODO: restrict instigator_py Django project initialization
...
# It updates the instigator_py context to trim leading and trailing spaces
# from domain/email values
"""
{{ instigator_py.update({ "domain_name": instigator_py.domain_name | trim }) }}
{{ instigator_py.update({ "email": instigator_py.email | trim }) }}
"""

project_slug = "{{ instigator_py.project_slug }}"
...
assert "\\" not in "{{ instigator_py.author_name }}", "Don't include backslashes in author name."

if "{{ instigator_py.use_docker }}".lower() == "n":
    python_major_version = sys.version_info[0]
    if python_major_version == 2:
        print(
            WARNING + "You're running instigator_py under Python 2, but the generated "
...
if "{{ instigator_py.use_whitenoise }}".lower() == "n" and "{{ instigator_py.cloud_provider }}" == "None":
    print("You should either use Whitenoise or select a " "Cloud Provider to serve static files")
    sys.exit(1)

if "{{ instigator_py.mail_service }}" == "Amazon SES" and "{{ instigator_py.cloud_provider }}" != "AWS":
