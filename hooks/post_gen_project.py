...
NOTE:
    ...
    as the whole instigator_py Django project initialization
    can potentially be run in Python 2.x environment
    (at least so we presume in `pre_gen_project.py`).

TODO: restrict instigator_py Django project initialization to
...
def remove_docker_files():
    ...
    for file_name in file_names:
        os.remove(file_name)
    if "{{ instigator_py.editor }}".lower() == "pycharm":
...
def remove_heroku_files():
    ...
    for file_name in file_names:
        if file_name == "requirements.txt" and "{{ instigator_py.ci_tool }}".lower() == "travis":
...
def remove_sass_files():
    shutil.rmtree(os.path.join("{{instigator_py.project_slug}}", "static", "sass"))
...
def remove_celery_files():
    file_names = [
        ...
        os.path.join("{{ instigator_py.project_slug }}", "users", "tasks.py"),
        os.path.join("{{ instigator_py.project_slug }}", "users", "tests", "test_tasks.py"),
    ]
    for file_name in file_names:
        os.remove(file_name)
...
def set_flags_in_envs(database_user, celery_flower_user, debug=False):
    ...
    selected_database = "{{ instigator_py.database_engine }}"
...
def remove_drf_starter_files():
    ...
    # shutil.rmtree(os.path.join("{{instigator_py.project_slug}}", "users", "api"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "api", "serializers.py"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "api", "views.py"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "tests", "test_drf_urls.py"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "tests", "test_drf_views.py"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "tests", "test_swagger.py"))


def remove_graphene_starter_files():
    os.remove(os.path.join("config", "schema.py"))
    shutil.rmtree(os.path.join("api"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "api", "schema.py"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "api", "queries.py"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "api", "mutations.py"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "api", "filters.py"))
    os.remove(os.path.join("{{instigator_py.project_slug}}", "users", "api", "inputs.py"))
...
def remove_tenant_files():
    ...
    shutil.rmtree(os.path.join("{{instigator_py.project_slug}}", "client"))


def remove_storages_module():
    os.remove(os.path.join("{{instigator_py.project_slug}}", "utils", "storages.py"))


def main():
    debug = "{{ instigator_py.debug }}".lower() == "y"
    ...
    if "{{ instigator_py.open_source_license}}" != "GPLv3":
        remove_gplv3_files()

    if "{{ instigator_py.username_type }}" == "username":
    ...
    if "{{ instigator_py.use_docker }}".lower() == "y":
        if "{{ instigator_py.database_engine }}".lower() == "postgresql":
            remove_mysql_docker_folder()
        elif "{{ instigator_py.database_engine }}".lower() == "mysql":
    ...
    if "{{ instigator_py.use_docker }}".lower() == "y" and "{{ instigator_py.cloud_provider}}" != "AWS":
        remove_aws_dockerfile()

    if "{{ instigator_py.use_heroku }}".lower() == "n":
        remove_heroku_files()
    elif "{{ instigator_py.frontend_pipeline }}" != "Django Compressor":
        remove_heroku_build_hooks()

    if "{{ instigator_py.use_docker }}".lower() == "n" and "{{ instigator_py.use_heroku }}".lower() == "n":
        if "{{ instigator_py.keep_local_envs_in_vcs }}".lower() == "y":
            print(
    ...
    if "{{ instigator_py.frontend_pipeline }}" in ["None", "Django Compressor"]:
        remove_gulp_files()
        remove_webpack_files()
        remove_sass_files()
        ...
        handle_js_runner(
            "{{ instigator_py.frontend_pipeline }}",
            use_docker=("{{ instigator_py.use_docker }}".lower() == "y"),
            use_async=("{{ instigator_py.use_async }}".lower() == "y"),
        )

    if "{{ instigator_py.cloud_provider }}" == "None" and "{{ instigator_py.use_docker }}".lower() == "n":
        print(
    ...
    if "{{ instigator_py.use_celery }}".lower() == "n":
        remove_celery_files()
        if "{{ instigator_py.use_docker }}".lower() == "y":
            remove_celery_compose_dirs()

    if "{{ instigator_py.ci_tool }}" != "Travis":
        remove_dottravisyml_file()

    if "{{ instigator_py.ci_tool }}" != "Gitlab":
        remove_dotgitlabciyml_file()

    if "{{ instigator_py.ci_tool }}" != "Github":
        remove_dotgithub_folder()

    if "{{ instigator_py.use_drf }}".lower() == "n":
        remove_drf_starter_files()

    if "{{ instigator_py.use_graphene }}".lower() == "n":
        remove_graphene_starter_files()

    if "{{ instigator_py.use_async }}".lower() == "n":
        remove_async_files()
        
    if "{{ instigator_py.use_tenants }}".lower() == "n":
        remove_tenant_files()
