...
If you are using ``supervisor``, you can use this file as a starting point::
    ...
    [program:{{instigator_py.project_slug}}]
    command=docker-compose -f production.yml up
    directory=/path/to/{{instigator_py.project_slug}}
    redirect_stderr=true
    autostart=true
    autorestart=true
    priority=10

Move it to ``/etc/supervisor/conf.d/{{instigator_py.project_slug}}.conf`` and run::

    supervisorctl reread
    supervisorctl update
    supervisorctl start {{instigator_py.project_slug}}
